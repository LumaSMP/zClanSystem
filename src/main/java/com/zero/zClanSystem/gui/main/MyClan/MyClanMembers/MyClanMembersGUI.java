...
public class MyClanMembersGUI {
    ...
    public MyClanMembersGUI(ClanManager clanManager, Clan clan) {
    ...
    public void open(Player player, int page) {
        ...
        Inventory inv = Bukkit.createInventory(
                new GUIHolder(GUIType.MY_CLAN_MEMBERS_LIST, clan),
                54,
                title
        );
        ...
        String query = MyClanMembersSearch.searchMap.get(uuid);
    ...
    private void fillBottom(Player player, Inventory inv, int page, boolean hasPrev, boolean hasNext) {
        ...
        inv.setItem(49, GUIUtils.item(Material.ARROW, "§eBack", "§7Return to Menu"));

        if (hasNext) inv.setItem(53, GUIUtils.item(Material.ARROW, "§eNext Page"));

        String query = MyClanMembersSearch.searchMap.get(player.getUniqueId());
